...
class HistoryChart extends Component {
  ...
  render() {
    ...
    const { startDate, endDate } = this.props.analysisPeriod
    ...
    if (sensorType === 'air' && airSensor !== undefined) {
      if (airSensor.historyPM2_5) {
        for (let i = 0, l = airSensor.historyPM2_5.length; i < l; i++) {
          if (moment(airSensor.historyPM2_5[i].x, "DD-MM-YYYY HH:mm:ss").isSameOrAfter(moment(startDate), 'day') &&
          moment(airSensor.historyPM2_5[i].x, "DD-MM-YYYY HH:mm:ss").isSameOrBefore(moment(endDate), 'day')) {
      ...
      if (airSensor.historyPM10) {
        for (let i = 0, l = airSensor.historyPM10.length; i < l; i++) {
          if (moment(airSensor.historyPM10[i].x, "DD-MM-YYYY HH:mm:ss").isSameOrAfter(moment(startDate), 'day') &&
          moment(airSensor.historyPM10[i].x, "DD-MM-YYYY HH:mm:ss").isSameOrBefore(moment(endDate), 'day')) {
    ...
    if (sensorType === 'visual' && visualSensor !== undefined) {
      if (visualSensor.historyPedestrian) {
        for (let i = 0, l = visualSensor.historyPedestrian.length; i < l; i++) {
          if (moment(visualSensor.historyPedestrian[i].x, "DD-MM-YYYY HH:mm:ss").isSameOrAfter(moment(startDate), 'day') &&
          moment(visualSensor.historyPedestrian[i].x, "DD-MM-YYYY HH:mm:ss").isSameOrBefore(moment(endDate), 'day')) {
      ...
      if (visualSensor.historyBicycle) {
        for (let i = 0, l = visualSensor.historyBicycle.length; i < l; i++) {
          if (moment(visualSensor.historyBicycle[i].x, "DD-MM-YYYY HH:mm:ss").isSameOrAfter(moment(startDate), 'day') &&
          moment(visualSensor.historyBicycle[i].x, "DD-MM-YYYY HH:mm:ss").isSameOrBefore(moment(endDate), 'day')) {
      ...
      if (visualSensor.historyVehicle) {
        for (let i = 0, l = visualSensor.historyVehicle.length; i < l; i++) {
          if (moment(visualSensor.historyVehicle[i].x, "DD-MM-YYYY HH:mm:ss").isSameOrAfter(moment(startDate), 'day') &&
          moment(visualSensor.historyVehicle[i].x, "DD-MM-YYYY HH:mm:ss").isSameOrBefore(moment(endDate), 'day')) {
